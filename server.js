    ...
    try {
      ...
      flows.forEach((f) => {
        ...
        if (!byDev[devKey]) {
          byDev[devKey] = {
            device: {
              id: f.device || f.deviceIP,
              name: nameMap[devKey] || f.deviceIP || devKey,
              ip: f.deviceIP || '',
            },
            download: 0,
            upload: 0,
        ...
        byDev[devKey].download += f.download || 0;
        byDev[devKey].upload += f.upload || 0;
        byDev[devKey].total += (f.download || 0) + (f.upload || 0);
      });
